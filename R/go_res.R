# analysis of GO terms from single category with calculation for parent GO terms
go_res <- function(genes, background, p_value = 0.05, category = "BP", rrvgo_threshold=0.8) {
    require (gprofiler2)
    require (rrvgo)
    
    #enrichment calculation
    res <- gost(query = genes, organism = "athaliana", 
    ...
    simMatrix <- calculateSimMatrix(category_selected$term_id,
